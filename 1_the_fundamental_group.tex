...
\begin{example}[Straight Line Homotopy]
	Let $V$ be a real vector space. A subset $S \subseteq V$ is said to be \bld{convex}, if the line segment $\cbr[0]{(1 - t)p + tq : 0 \leq t \leq 1}$ is contained in $S$ for all $p,q \in V$. Suppose now that $V$ is finite dimensional and $S \subseteq V$ is convex. Moreover, let $f,g \in \mathsf{Top}(X,S)$ for some $X \in \ob(\mathsf{Top})$. Define $H : X \times I \to S$ by
	\begin{equation*}
		H(x,t) := (1 - t)f(x) + tg(x).
	\end{equation*}
	Then $H$ is continuous and clearly $H : f \simeq g$. We call $H$ the \bld{straight line homotopy between $f$ and $g$}. Hence any two continuous maps defined on the same domain into a convex space are freely homotopic.
\end{example}
...
\begin{definition}[Nullhomotopic]
	A mapping $f \in \mathsf{Top}(X,Y)$ is said to be \bld{nullhomotopic}, if $f$ is freely homotopic to a constant map.
\end{definition}

\begin{definition}[Contractible]
	A topological space $X$ is said to be \bld{contractible}, if $\id_X$ is nullhomotopic.
\end{definition}

\begin{definition}[Reparametrization]
	Let $u$ be a path in a topological space $X$. A \bld{reparametrization} of $u$ is a path $u \circ \varphi$, where $\varphi \in \mathsf{Top}(I,I)$ fixing $0$ and $1$.
\end{definition}
...
\begin{definition}[Simply Connected]
	A path connected topologial space $X$ is said to be \bld{simply connected}, if $\pi_1(X)$ is trivial.
\end{definition}
...
\begin{proof}
	The idea is to consider first the case $F = \id_{I \times I}$. Hence define the paths $f_0$, $f_1$, $g_0$ and $g_1$ in $I \times I$ as indicated in figure \ref{fig:square_lemma_paths}. Then there is a straight line homotopy $H : I \times I \to I \times I$ between them as indicated in figure \ref{fig:square_lemma_homotopy}. Explicitly
	\begin{equation*}
	...
	\begin{equation*}
		(F \circ H)(s,t) = \ccases{
			F\del[1]{2st,2s(1-t)} & 0 \leq s \leq \frac{1}{2},\\
			F\del[1]{t + (1 - t)(2s - 1),1 + 2t(s - 1)} & \frac{1}{2} \leq s \leq 1,	
		}
	\end{equation*}
	...
	\begin{figure}[h!tb]
		...
		\begin{subfigure}[b]{.5\textwidth}
			...
			\begin{tikzpicture}[scale = 3]
				...
				\draw (.5,0) node[below] {$g_0$};
				\draw (1,.5) node[right] {$g_1$};
				\draw (0,.5) node[left] {$f_0$};
				\draw (.5,1) node[above] {$f_1$};
			\end{tikzpicture}
...
\begin{definition}[$\mathsf{hTop}$]
	The quotient category under the congruence of being freely homotopic is called the \bld{homotopy category}, and is denoted by $\mathsf{hTop}$.
\end{definition}

\begin{definition}[Homotopy Type]
	Two topological spaces $X$ and $Y$ are of the \bld{same homotopy type}, if they are isomorphic in $\mathsf{hTop}$. An explicit choice of such an isomorphism is called a \bld{homotopy equivalence}.
\end{definition}
...
\begin{theorem}[Homotopy Invariance of $\pi_1$]
